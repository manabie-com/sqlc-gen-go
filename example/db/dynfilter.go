...
func dynSplitPlaceholders(text string) (parts []string, argNums []int) {
	for {
		...
		if j == i+1 {
			// '$' not followed by digit; keep scanning.
			// Rare edge-case: just advance past the '$'.
			i++
			continue
		}
...
// dynFinalizeQuery strips a dangling trailing comma from the last non-empty
// line and removes an orphaned ORDER BY (all its items were filtered out).
func dynFinalizeQuery(query string) string {
	...
	lastLine := query[start:end]
	trimmed := strings.TrimSpace(lastLine)

	// Strip trailing comma.
	if len(trimmed) > 0 && trimmed[len(trimmed)-1] == ',' {
		// Find the comma position in lastLine.
		commaPos := strings.LastIndexByte(lastLine, ',')
		if commaPos >= 0 {
			query = query[:start+commaPos] + query[start+commaPos+1:]
			end--
		}
		// Recalculate trimmed after removal.
		lastLine = query[start:end]
		trimmed = strings.TrimSpace(lastLine)
	}

	// Remove orphaned ORDER BY.
