...
func TestParseDynFilter_AnyWithoutArrayType(t *testing.T) {
	sql := "SELECT * FROM items\nWHERE id = ANY($1) -- :if @ids"
	params := []*plugin.Parameter{
		{Number: 1, Column: &plugin.Column{Name: "ids", NotNull: true, Type: &plugin.Identifier{Name: "bigint"}}},
	}
	_, err := ParseDynFilter(sql, params)
	if err == nil {
		t.Fatal("expected error for ANY(@ids) without array type")
	}
	if !strings.Contains(err.Error(), "ANY(@ids) requires an array type cast") {
		t.Errorf("unexpected error message: %s", err)
	}
}

func TestParseDynFilter_AnyWithArrayType(t *testing.T) {
	sql := "SELECT * FROM items\nWHERE id = ANY($1) -- :if @ids"
	params := []*plugin.Parameter{
...
func TestParseDynFilter_AnyWithSqlcSlice(t *testing.T) {
	sql := "SELECT * FROM items\nWHERE id = ANY($1) -- :if @ids"
	params := []*plugin.Parameter{
		{Number: 1, Column: &plugin.Column{Name: "ids", IsSqlcSlice: true, NotNull: true, Type: &plugin.Identifier{Name: "bigint"}}},
	}
	info, err := ParseDynFilter(sql, params)
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
